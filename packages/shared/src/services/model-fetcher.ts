...
export const modelListCache = new Map<string, { expiresAt: number; models: ModelTemplate[] }>();

export function buildModelId(agentType: string, model?: string): string {
    return `${agentType}${MODEL_ID_SEPARATOR}${model ?? ''}`;
}

export function parseModelId(modelId: string): { agentType: string; model?: string } | null {
 ...
 */
export async function fetchGeminiModels(apiKey: string, baseUrl?: string): Promise<string[]> {
