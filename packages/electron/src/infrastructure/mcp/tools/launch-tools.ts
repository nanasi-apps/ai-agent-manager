...
Use this when implementation is complete and you want to start the development server.`,
			inputSchema: {
				timeout: z
					...
					.optional()
					.describe(
						"Timeout in milliseconds to wait for readiness (default: 60000)",
					),
			},
		},
		async (args): Promise<ToolResult> => {
			...
			try {
				...
				console.log(
					`[McpServer] launch_project: sessionContext=${JSON.stringify(context)}`,
				);
				...
				if (context?.sessionId) {
					...
					console.log(
						`[McpServer] launch_project: getSessionCwd(${context.sessionId}) = ${worktreeCwd ?? "undefined"}`,
					);
					if (worktreeCwd) {
						console.log(`[McpServer] Using worktree path: ${worktreeCwd}`);
					} else {
						console.log(
							`[McpServer] No worktree path found for session ${context.sessionId}`,
						);
					}
				} else {
...
You can now use 'launch_project' to start this application.`,
	...
	registerTool(
		...
		{
			description:
				"Stop a running project that was launched with launch_project.",
			inputSchema: {},
		},
		async (): Promise<ToolResult> => {
			...
			try {
				const context = getSessionContext();
				const stopped = await devServerManager.stopProject(
					projectId,
					context?.sessionId,
				);
				if (stopped) {
					...
					return {
						content: [
							{ type: "text", text: `Project ${projectId} is not running` },
						],
					};
				}
			} catch (error) {
	...
	registerTool(
		...
		{
			description:
				"List all currently running projects launched via launch_project",
			inputSchema: {},
		},
		async (): Promise<ToolResult> => {
			...
			if (processes.length === 0) {
				return {
					content: [
						{ type: "text", text: "No projects are currently running." },
					],
				};
			}
