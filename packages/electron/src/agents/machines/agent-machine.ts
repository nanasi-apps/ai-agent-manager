...
export type AgentEvents = 
  ...
  | { type: "SET_AGENT_DATA"; data: { geminiHome?: string; claudeHome?: string; invalidGeminiSession?: boolean } }
...
export const agentMachine = setup({
  ...
  actions: {
    ...
    setLastUserMessage: assign({
      lastUserMessage: ({ event }) => event.type === "USER_MESSAGE" ? event.message : undefined
    }),
    updateConfig: assign({
    ...
    setGeminiSession: assign({
      geminiSessionId: ({ event }) => event.type === "SET_GEMINI_SESSION" ? event.id : undefined
    }),
    ...
    setAgentData: assign({
      geminiHome: ({ context, event }) => event.type === "SET_AGENT_DATA" && event.data.geminiHome !== undefined ? event.data.geminiHome : context.geminiHome,
      claudeHome: ({ context, event }) => event.type === "SET_AGENT_DATA" && event.data.claudeHome !== undefined ? event.data.claudeHome : context.claudeHome,
      invalidGeminiSession: ({ context, event }) => event.type === "SET_AGENT_DATA" && event.data.invalidGeminiSession !== undefined ? event.data.invalidGeminiSession : context.invalidGeminiSession,
    }),
    setPendingHandover: assign({
      pendingHandover: ({ event }) => event.type === "SET_PENDING_HANDOVER" ? event.context : undefined
    }),
    consumePendingHandover: assign({
      pendingHandover: undefined
    }),
    setPendingWorktreeResume: assign({
      pendingWorktreeResume: ({ event }) => event.type === "SET_PENDING_WORKTREE_RESUME" ? event.pending : undefined
    }),
    clearPendingWorktreeResume: assign({
      pendingWorktreeResume: undefined
    }),
    activateWorktree: assign({
      activeWorktree: ({ event }) => event.type === "ACTIVATE_WORKTREE" ? event.context : undefined,
      config: ({ context, event }) => event.type === "ACTIVATE_WORKTREE" ? { ...context.config, cwd: event.context.cwd } : context.config
    }),
    clearActiveWorktree: assign({
      activeWorktree: undefined,
      config: ({ context }) => ({ ...context.config, cwd: context.projectRoot }) // Fallback to project root
    }),
  }
}).createMachine({
  id: "agent",
  ...
  states: {
    idle: {
      on: {
        ...
        CLEAR_PENDING_WORKTREE_RESUME: { actions: "clearPendingWorktreeResume" },
        ...
        RESET: {
          ...
          actions: assign(({ context, event }) => {
             const baseUpdate = {
               messageCount: 0,
               buffer: "",
               config: event.config ? { ...context.config, ...event.config } : context.config,
             };
