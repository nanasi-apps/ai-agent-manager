...
import { detectAgentType, supportsMcpIntegration } from "./agent-type-utils";
...
export class OneShotAgentManager extends EventEmitter implements IAgentManager {
	...
	async sendToSession(sessionId: string, message: string) {
		...
		if (session.messageCount === 0) {
			...
			if (baseRules) {
				console.log(
					`[OneShotAgentManager] Injecting rules for session ${sessionId}`,
				);
			}
		}
		...
		try {
			...
			// Use centralized agent type detection
			const agentTypeInfo = detectAgentType(session.config);
			const { isGemini, isCodex, isClaude } = agentTypeInfo;
			...
			const context: AgentDriverContext = {
				...
				mcpServerUrl: supportsMcpIntegration(agentTypeInfo)
					? MCP_SERVER_URL
					: undefined,
			};
			...
			// Execute the command
			console.log(
				`[OneShotAgentManager] Running: ${cmd.command} ${cmd.args.join(" ")}`,
			);
			...
			if (session.config.cwd) {
				try {
					...
					console.error(
						`[OneShotAgentManager] CWD ${session.config.cwd} does not exist.`,
					);
					if (
						session.projectRoot &&
						...
						console.log(
							`[OneShotAgentManager] Falling back to project root: ${session.projectRoot}`,
						);
						this.emitLog(
							sessionId,
							`[Warning] Worktree directory ${session.config.cwd} not found. Falling back to project root.\n`,
							"system",
						);
						...
						this.emitLog(
							sessionId,
							`[Error] CWD ${session.config.cwd} does not exist and no valid fallback.\n`,
							"error",
						);
						session.isProcessing = false;
						return;
			...
			if (child.stderr) {
				child.stderr.on("data", (data) => {
					...
					if (
						str.includes("Invalid session identifier") ||
						str.includes("No previous sessions found")
					) {
						if (session.geminiSessionId) {
							console.warn(
								`[OneShotAgentManager ${sessionId}] Gemini session ${session.geminiSessionId} is invalid, clearing it.`,
							);
							session.geminiSessionId = undefined;
						}
					...
					if (str.includes("Error when talking to Gemini API")) {
						...
						if (str.includes("exhausted your capacity") || quotaMatch) {
							...
							if (session.geminiSessionId) {
								console.warn(
									`[OneShotAgentManager ${sessionId}] Gemini API error during session ${session.geminiSessionId}, clearing session ID for retry.`,
								);
								session.geminiSessionId = undefined;
							}
			...
			child.on("close", (code) => {
				...
				session.isProcessing = false;
				console.log(
					`[OneShotAgentManager ${sessionId}] Process exited with code ${code}`,
				);
			...
			child.on("error", (err) => {
				...
				session.isProcessing = false;
				console.error(
					`[OneShotAgentManager ${sessionId}] Failed to start subprocess.`,
					err,
				);
				this.emitLog(
					sessionId,
	...
	requestWorktreeResume(
		...
		const resumeMessage =
			...
			this.buildWorktreeResumeMessage(session, request);
	...
	private buildWorktreeInstructions(session: SessionInfo): string {
		...
		lines.push(
			`- Use repoPath = Project root. Suggested branch: ${branchSuggestion}.`,
		);
		lines.push(
	...
	private appendWorktreeReminder(
		session: SessionInfo,
		message: string,
	): string {
		...
		reminderLines.push(
			`Use repoPath = Project root, branch = ${branchSuggestion}.`,
		);
		reminderLines.push(
	...
	private buildWorktreeResumeMessage(
		session: SessionInfo,
		request: WorktreeResumeRequest,
	): string {
	...
	private async resolveSessionCwd(
		sessionId: string,
		session: SessionInfo,
	...
	private async validateActiveWorktree(
		sessionId: string,
		session: SessionInfo,
	): Promise<void> {
			...
			try {
				...
				for (const log of logs) {
					...
					if (log.metadata?.geminiSessionId && !session.invalidGeminiSession) {
						...
						console.log(
							`[OneShotAgentManager] Captured Gemini session ID: ${session.geminiSessionId}`,
						);
					}
					...
					if (log.metadata?.codexThreadId) {
						...
						console.log(
							`[OneShotAgentManager] Captured Codex thread ID: ${session.codexThreadId}`,
						);
					}
