...
export class UnifiedAgentManager extends EventEmitter implements IAgentManager {
    ...
    startSession(sessionId: string, command: string, config?: Partial<AgentConfig>) {
        console.log(`[UnifiedAgentManager] Starting session ${sessionId} (type: ${config?.type ?? 'custom'})`);
    ...
    requestWorktreeResume(sessionId: string, request: WorktreeResumeRequest): boolean {
        return oneShotAgentManager.requestWorktreeResume?.(sessionId, request) ?? false;
    }
}
