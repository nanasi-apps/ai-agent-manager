...
export const GEMINI_QUOTA_PATTERN = /Your quota will reset after (\d+h\d+m\d+s|\d+m\d+s|\d+s)/;
...
export function isQuotaError(stderr: string): boolean {
    return stderr.includes(GEMINI_CAPACITY_PATTERN) || GEMINI_QUOTA_PATTERN.test(stderr);
}
