	...
	async stopProject(
		...
		try {
			...
			try {
				...
				if (e.code !== "ESRCH") {
					logger.info("Failed to kill process group {pid}: {err}", {
						pid: running.pid,
						err: e,
					});
				}
	...
	async stopAll(): Promise<void> {
		...
		for (const process of this.runningProcesses.values()) {
			if (process.status === "running") {
				promises.push(
					this.stopProject(process.projectId, process.conversationId),
				);
			}
		}
	...
	async launchProject(
		...
		logger.info(
			"Launching project {projectId} (conversation: {conversationId}, cwd: {cwd}, config: {configName})",
			{
				...
				cwd: overrideCwd ?? "default",
				configName: configName ?? "auto",
			},
		);
		...
		try {
			...
			if (configName) {
				...
				if (!config)
					throw new Error(
						`Launch configuration "${configName}" not found for project "${project.name}"`,
					);
			} else {
				// Fallback / Auto logic
			...
			if (!config)
				throw new Error(
					`Project "${project.name}" does not have Agent Configuration set up.`,
				);
			if (!project.rootPath)
				throw new Error(
					`Project "${project.name}" does not have a root path set`,
				);
			...
			logger.info("Spawning: {fullCommand} in {workingDir}", {
				fullCommand,
				workingDir,
			});
			...
			if (!childProcess.pid)
				throw new Error(`Failed to spawn process for command: ${cmd}`);
			...
			logger.error("Error launching project {projectId}: {error}", {
				projectId,
				error,
			});
			// Cleanup on error
