import type { IWebServerService, IWebServerStatus } from "@agent-manager/shared";
...
class WebServerManager implements IWebServerService {
    ...
    async start(options?: { port?: number; host?: string }): Promise<IWebServerStatus> {
        if (this.server) {
        ...
        if (!port) {
            port = await getPort();
            logger.info("No port specified. Using random available port: {port}", { port });
        } else {
        ...
        try {
            ...
            logger.info("Web Server started successfully at {localUrl}, {networkUrl}", { localUrl, networkUrl });
        } catch (error) {
