...
import { formatOutput, runCommand, runGit } from "../utils";
...
async function runConflictCheck(
	...
	if (!depCruiseBin) {
		return {
			status: "skipped",
			output:
				"dependency-cruiser not installed (node_modules/.bin/depcruise not found).",
		};
	}
		...
		return [
			{
				...
				description:
					"Run a quick check, commit staged changes, and run a conflict check",
				inputSchema: {
					type: "object",
					properties: {
						repoPath: {
						...
						files: {
							type: "array",
							items: { type: "string" },
							description: "Files to stage (default: stage all)",
						},
						checkCommand: {
							type: "string",
							description:
								"Optional command to run before commit (default: git diff --check --cached)",
						},
						checkArgs: {
							type: "array",
							items: { type: "string" },
							description: "Extra args for checkCommand",
						},
						...
						conflictPaths: {
							type: "array",
							items: { type: "string" },
							description: "Paths for dependency-cruiser conflict check",
						},
						conflictArgs: {
							type: "array",
							items: { type: "string" },
							description: "Extra args for dependency-cruiser",
						},
						conflictConfig: {
							type: "string",
							description: "Dependency-cruiser config file path",
						},
					},
					required: ["repoPath"],
				},
			...
			},
			{
				name: "auto_rebase",
				description:
					"Auto rebase the current branch onto its upstream or specified base",
				inputSchema: {
					type: "object",
					properties: {
						repoPath: {
						...
						upstream: {
							type: "string",
							description: "Upstream branch (defaults to tracking branch)",
						},
						fetch: {
							type: "boolean",
							description: "Run git fetch before rebase (default: true)",
						},
					},
					required: ["repoPath"],
				},
				...
				inputSchema: {
					...
					properties: {
						...
						extraArgs: {
							type: "array",
							items: { type: "string" },
							description: "Extra args to pass to dependency-cruiser",
						},
					},
					required: ["repoPath"],
				},
	...
	async callTool(name: string, args: any): Promise<any> {
		...
		switch (name) {
			case "commit_and_sync": {
				...
				const stagedFilesRaw = await runGit(repoPath, [
					"diff",
					"--cached",
					"--name-only",
				]);
				...
				let checkOutput = "";
				if (
					typeof args.checkCommand === "string" &&
					args.checkCommand.trim().length > 0
				) {
				...
				if (args.push) {
					...
					const branch =
						args.branch ||
						(await runGit(repoPath, ["rev-parse", "--abbrev-ref", "HEAD"]));
			...
			case "auto_rebase": {
				...
				if (!upstream) {
					try {
						upstream = await runGit(repoPath, [
							"rev-parse",
							"--abbrev-ref",
							"--symbolic-full-name",
							"@{u}",
						...
						throw new Error(
							"No upstream configured. Provide upstream parameter.",
						);
					}
				}
