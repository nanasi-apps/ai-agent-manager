...
export class AgentOrchestrationProvider implements InternalToolProvider {
    async listTools(): Promise<McpTool[]> {
        return [
            {
                name: "dispatch_task",
                description: "Dispatch a task to an agent session (starts session if not running)",
                inputSchema: {
                    type: "object",
                    properties: {
                        sessionId: { type: "string", description: "Target session ID" },
                        message: { type: "string", description: "Task description or message" },
                        agentType: { type: "string", description: "Agent type (e.g. 'gemini', 'claude', 'custom') - required if starting new session" },
                        command: { type: "string", description: "Command to run agent - overrides agentType default" },
                        agentModel: { type: "string", description: "Model name" },
                        cwd: { type: "string", description: "Working directory for the agent" },
                        env: { type: "object", description: "Environment variables" },
                        streamJson: { type: "boolean", description: "Whether the agent streams JSON (default: false)" }
                    },
            ...
            },
            {
                ...
                inputSchema: {
                    ...
                    properties: {
                        ...
                        sessionIds: { type: "array", items: { type: "string" }, description: "Specific sessions to target (default: all)" }
                    },
                    required: ["message"]
                },
                serverName: "agents-manager-mcp"
