	...
	async listResources(): Promise<McpResource[]> {
		...
		for (const repoPath of roots) {
			try {
				...
				console.warn(
					`[WorktreeResourceProvider] Failed to list worktrees for ${repoPath}`,
					error,
				);
			}
		}
	...
	async readResource(uri: string): Promise<McpResourceContent> {
		...
		const targetRoots = lookup.repoPath
			...
			: lookup.projectId
				? (store
						.listProjects()
						.filter((project) => project.id === lookup.projectId)
						.map((project) => project.rootPath)
						.filter(Boolean) as string[])
		...
		const matches: Array<{
			repoPath: string;
			worktree: Awaited<
				ReturnType<typeof worktreeManager.getWorktrees>
			>[number];
		}> = [];
		...
		for (const repoPath of targetRoots) {
			try {
				...
				console.warn(
					`[WorktreeResourceProvider] Failed to read worktrees for ${repoPath}`,
					error,
				);
			}
		}
		...
		if (matches.length > 1) {
			...
			throw new Error(
				`Multiple worktrees match branch ${lookup.branch}. Use ?repoPath= to disambiguate. Matches: ${roots}`,
			);
		}
