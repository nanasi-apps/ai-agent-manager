...
export class McpConnectionManager extends EventEmitter {
    ...
    async connect(config: McpServerConfig) {
        if (this.connections.has(config.name)) {
            console.warn(`[McpConnectionManager] Server ${config.name} already connected.`);
            return;
        }

        console.log(`[McpConnectionManager] Connecting to server: ${config.name} (${config.command} ${config.args.join(' ')})`);
        ...
        try {
            ...
            console.error(`[McpConnectionManager] Failed to connect to ${config.name}:`, error);
            throw error;
        }
    }
    ...
    async disconnect(name: string) {
        ...
        if (connection) {
            try {
                ...
                console.error(`[McpConnectionManager] Error closing client for ${name}:`, error);
            }
