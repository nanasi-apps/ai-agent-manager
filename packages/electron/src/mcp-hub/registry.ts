	...
	async listTools(): Promise<McpTool[]> {
		...
		for (const [name, providers] of this.internalProviders) {
			for (const provider of providers) {
				try {
					...
					console.error(
						`[McpRegistry] Error listing internal tools for ${name}:`,
						err,
					);
				}
			}
		...
		for (const connection of this.connectionManager.getAllConnections()) {
			try {
				...
				console.error(
					`[McpRegistry] Error listing tools for ${connection.config.name}:`,
					err,
				);
			}
		}
	...
	async listResources(): Promise<McpResource[]> {
		...
		for (const [name, providers] of this.internalProviders) {
			for (const provider of providers) {
				...
				try {
					...
					console.error(
						`[McpRegistry] Error listing internal resources for ${name}:`,
						err,
					);
				}
			}
		...
		for (const connection of this.connectionManager.getAllConnections()) {
			try {
				...
				console.error(
					`[McpRegistry] Error listing resources for ${connection.config.name}:`,
					err,
				);
			}
		}
	...
	async listResourceTemplates(): Promise<McpResourceTemplate[]> {
		...
		for (const [name, providers] of this.internalProviders) {
			for (const provider of providers) {
				...
				try {
					...
					console.error(
						`[McpRegistry] Error listing internal resource templates for ${name}:`,
						err,
					);
				}
			}
		...
		for (const connection of this.connectionManager.getAllConnections()) {
			try {
				...
				console.error(
					`[McpRegistry] Error listing resource templates for ${connection.config.name}:`,
					err,
				);
			}
		}
