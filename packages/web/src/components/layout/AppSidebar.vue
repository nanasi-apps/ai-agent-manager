	...
	<Sidebar collapsible="icon" class="border-r relative select-none">
		...
		<SidebarContent>
					...
					<SidebarMenu>
						<SidebarMenuItem v-for="item in navItems" :key="item.id">
							...
							>
								<router-link
									:to="item.url"
									class="flex items-center gap-2 !text-inherit grow overflow-hidden"
								>
			...
			<SidebarGroup class="group-data-[collapsible=icon]:hidden">
				...
				>
					<Plus/>
					<span class="sr-only">{{ t('sidebar.addProject') }}</span>
				</SidebarGroupAction>
				<SidebarGroupContent>
					<SidebarMenu>
						<Collapsible
									...
									>
										<div class="flex items-center gap-2">
											<CollapsibleTrigger as-child>
												<div
													class="size-4 shrink-0 cursor-pointer flex items-center justify-center relative"
												>
													<Folder
														class="size-4 transition-opacity group-hover/collapsible:opacity-0"
													/>
													<ChevronRight
											...
											<Loader2
												v-if="project.conversations.some(c => c.isRunning)"
												class="size-3 animate-spin shrink-0 text-muted-foreground mr-1"
											/>
										</div>
									</SidebarMenuButton>
									<SidebarMenuAction
										class="absolute right-1 top-1.5"
										@click.stop="openNewConversion(project.id)"
									>
										<Plus/>
										<span class="sr-only"
											>{{ t('sidebar.newConversion') }}</span
										>
									</SidebarMenuAction>
								</div>
								...
								<CollapsibleContent class="project-collapsible">
									<div class="project-collapsible__inner">
										<SidebarMenuSub v-if="project.conversations.length">
											<SidebarMenuSubItem
												v-for="conv in project.conversations.slice(0, 5)"
												:key="conv.id"
											>
												<SidebarMenuSubButton
													as-child
												...
												>
													<router-link
														:to="`/conversions/${conv.id}`"
														class="flex items-center justify-between gap-1 w-full overflow-hidden"
													>
										...
										<SidebarMenuSub v-else>
											<SidebarMenuSubItem>
												<span class="text-xs text-muted-foreground px-2 py-1"
													>{{ t('sidebar.noConversations') }}</span
												>
											</SidebarMenuSubItem>
										</SidebarMenuSub>
									</div>
				...
				<SidebarMenuItem v-for="item in bottomItems" :key="item.id">
					...
					>
						...
						>
							...
							<div
								v-if="item.isRunning"
								class="size-2 rounded-full bg-green-500 animate-pulse ml-auto mr-1 shrink-0 shadow-[0_0_8px_rgba(34,197,94,0.6)]"
							/>
						</router-link>
					</SidebarMenuButton>
				</SidebarMenuItem>
...
.project-collapsible {
	...
	--animate-collapsible-down: collapsible-down 0.28s
