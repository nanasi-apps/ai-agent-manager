	...
	<Dialog :open="store.isOpen" @update:open="(val) => !val && store.close()">
		<DialogContent class="max-w-xl">
			...
			<div v-if="request" class="space-y-4">
				...
				<p class="text-xs text-muted-foreground">
					ブランチ名は作業内容に紐づけてください (例:
					feature/os-notification-for-branch-prompt)。
				</p>
			...
			<DialogFooter>
				<Button variant="secondary" @click="store.close">Later</Button>
				<Button :disabled="!store.canSubmit" @click="store.submit">
