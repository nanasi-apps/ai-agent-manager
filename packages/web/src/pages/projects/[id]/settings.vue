			...
			<div v-else>
				...
				<div class="mt-6 border rounded-lg p-4 bg-card/60">
					...
					<div class="space-y-6">
						...
						<div>
							...
							>
								<div
									...
									:key="rule.id"
									class="flex items-center space-x-2 border p-3 rounded-md bg-background"
								>
									<Checkbox
										:id="`rule-${rule.id}`"
						...
						<div>
							...
							<div
								class="flex h-[500px] border rounded-md overflow-hidden bg-background"
							>
								<!-- Sidebar List -->
								...
								<div
									v-else
									class="flex-1 flex items-center justify-center text-muted-foreground bg-muted/5"
								>
				...
				<div class="mt-6 border rounded-lg p-4 bg-card/60">
					...
					<p class="text-sm text-muted-foreground mb-4">
						Configure how new worktrees are initialized. These settings are
						stored in <code>.gtrconfig</code>in your project root.
					</p>
				...
				<div class="mt-6 border rounded-lg p-4 bg-card/60">
					...
					<p class="text-sm text-muted-foreground mb-4">
						Configure automated project startup. Define how the development
						server launches and when it's ready.
					</p>
  ...
  "readiness": { "logPattern": "Ready in" }
						...
						<div class="bg-muted/30 rounded-md p-4 text-sm space-y-3">
							...
							<div class="grid gap-2 text-muted-foreground">
								<div>
									<code class="text-foreground">type</code>: <code>"web"</code>
									(opens browser) or <code>"other"</code>(Electron, Android,
									CLI)
								</div>
								<div>
									<code class="text-foreground">startCommand</code>: Startup
									command (e.g., <code>"pnpm run dev"</code>)
								</div>
								<div>
									<code class="text-foreground">services</code>: Array of
									service configs with <code>name</code>, <code>envKey</code>,
									<code>default</code>port, and optional <code>argument</code>
									for CLI port injection
								</div>
								<div>
									<code class="text-foreground">readiness.logPattern</code>:
									Regex to detect when server is ready
								</div>
							</div>
							...
							<details class="mt-3">
								...
								<div class="mt-2 space-y-2 pl-4">
									<div>
										...
										<code class="block bg-background p-2 rounded mt-1 text-xs">
											{ "type": "web", "startCommand": "pnpm run dev",
											"services": [ { "name": "App", "envKey": "PORT",
											"default": 3000, "argument": "--port"}], "readiness": {
											"logPattern": "Ready in" }}
										</code>
									</div>
									<div>
										...
										<code class="block bg-background p-2 rounded mt-1 text-xs">
											{ "type": "web", "startCommand": "pnpm run dev",
											"services": [ { "name": "App", "envKey": "PORT",
											"default": 5173, "argument": "--port"}], "readiness": {
											"logPattern": "Local:" }}
										</code>
									</div>
									<div>
										...
										<code class="block bg-background p-2 rounded mt-1 text-xs">
											{ "type": "other", "startCommand": "pnpm run dev",
											"services": [ { "name": "API", "envKey": "API_PORT",
											"default": 3000}], "readiness": { "logPattern": "Electron
											app ready" }}
										</code>
									</div>
								</div>
				...
				<!-- Danger Zone Section -->
				<div
					class="mt-12 border border-destructive/30 rounded-lg p-4 bg-destructive/5"
				>
		...
		<Transition name="slide-up">
			<div
				...
				class="fixed bottom-6 right-6 z-50 flex gap-2"
			>
				<div
					class="bg-popover text-popover-foreground border shadow-lg rounded-lg p-2 flex items-center gap-2"
				>
					<span class="text-sm font-medium px-2 text-muted-foreground"
					...
					<Button
						...
						:disabled="isSavingProject || !nameDraft.trim()"
					>
